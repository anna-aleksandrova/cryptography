...
class SHA256:
    ...
    @classmethod
    def _gamma0(cls, x):
        return cls._rotr(x, 7) ^ cls._rotr(x, 18) ^ (x >> 3)

    @classmethod
    def _gamma1(cls, x):
        return cls._rotr(x, 17) ^ cls._rotr(x, 19) ^ (x >> 10)
    ...
    def hash(cls, message: bytes) -> str:
        padded_message = cls._pad(message)
        ...
        for i in range(0, len(padded_message), 64):
            ...
            w = list(struct.unpack(b'>16I', chunk))
            
            for j in range(16, 64):
                w.append((cls._gamma1(w[j-2]) + w[j-7] + cls._gamma0(w[j-15]) + w[j-16]) & 0xFFFFFFFF)
            
            a, b, c, d, e, f, g, h_val = h
            
            for j in range(64):
                t1 = (h_val + cls._sigma1(e) + cls._ch(e, f, g) + cls.K[j] + w[j]) & 0xFFFFFFFF
...
if __name__ == "__main__":
    ...
    custom_hash = SHA256.hash(message_bytes)
    standard_hash = hashlib.sha256(message_bytes).hexdigest()

    print(f"Message: '{text}'\n")
    print(f"SHA256: {custom_hash}")
    print(f"Hashlib: {standard_hash}")

    print(custom_hash == standard_hash)
